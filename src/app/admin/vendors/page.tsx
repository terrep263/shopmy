...
export default function AdminVendorsPage() {
  ...
  useEffect(() => {
    fetch("/api/admin/vendors", getAdminFetchOpts())
      .then(res => {
  ...
  return (
    <AdminLayout>
      <div className="dashHeader p-xl-5 p-4 pb-xl-0 pb-0 pt-lg-0 pt-5">
        <h2 className="fw-medium mb-0">Manage Vendors</h2>
      </div>
          ...
          <div className="col-12">
            <div className="card rounded-3 shadow-sm">
              <div className="card-header py-3 px-4 d-flex justify-content-between align-items-center">
                <h4 className="m-0">All Vendors</h4>
                <button className="btn btn-primary btn-sm">Add New Vendor</button>
              </div>
                  ...
                  <div className="table-responsive">
                    <table className="table table-hover mb-0">
                      ...
                      <tbody>
                        {data.map(vendor => (
                          <tr key={vendor.id}>
                            <td className="fw-medium">{vendor.business?.name || 'N/A'}</td>
                            <td>{vendor.user?.email || 'N/A'}</td>
                            <td><span className="badge bg-info">{vendor._count?.deals || 0} deals</span></td>
                            <td>
