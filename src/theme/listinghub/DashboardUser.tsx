/**
 * ListingHub Dashboard User — composed from ported theme components.
 * Mirrors: ListingHub/ListingHub_NextJs (Javascript)/src/app/dashboard-user/page.jsx
 */
'use client'
...
import AdminNavbar from '@/components/theme/navbar/admin-navbar'
import AdminSidebar from '@/components/theme/admin/admin-sidebar'
import RecentActivity from '@/components/theme/admin/recent-activity'
import BackToTop from '@/components/theme/back-to-top'

import { adminCounter, chatData, invoiceData } from '@/lib/theme-data'
              ...
              <div className="user-dashboard-box bg-light">
                ...
                <div className="dashCaption p-xl-5 p-3 p-md-4">
                  ...
                  <div className="row align-items-start g-4 mb-lg-5 mb-4">
                    {adminCounter.map((item: any, index: number) => {
                          ...
                          <div className="card rounded-3 position-relative p-4">
                            <div className={`position-absolute w-30 h-100 start-0 top-0 rounded-end-pill ${item.bg}`}>
                              <div className="position-absolute top-50 start-50 translate-middle">
                                <Icon className={`fs-2 ${item.iconStyle}`} />
                              </div>
                            </div>
                            ...
                            {chatData.map((item: any, index: number) => (
                              <div className="singleMessagesline" key={index}>
                                <Link href="#" className="d-flex align-items-center justify-content-start gap-2">
                                  <div className="msgLinethumb">
                                    <div className="square--50 circle">
                                      <Image src={item.image} width={0} height={0} sizes="100vw" style={{ width: '100%', height: '100%' }} className="img-fluid circle" alt="" />
                                    </div>
                                  </div>
                            ...
                            <tbody>
                              {invoiceData.map((item: any, index: number) => (
                                <tr key={index}>
                                  ...
                                  {item.status === 'Paid' && <td data-label="Status"><span className="badge badge-xs badge-success rounded-pill">Paid</span></td>}
                                  {item.status === 'Unpaid' && <td data-label="Status"><span className="badge badge-xs badge-danger rounded-pill">Unpaid</span></td>}
                                  {item.status === 'On Hold' && <td data-label="Status"><span className="badge badge-xs badge-info rounded-pill">On Hold</span></td>}
                                  <td data-label="Due Date"><span className="text-normal">{item.date}</span></td>
                                  <td data-label="View"><Link href="#" className="btn btn-sm fw-medium btn-light-primary rounded-pill">View Invoice</Link></td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                  ...
                  <div className="row align-items-start g-4">
                    <div className="col-xl-12 col-lg-12 col-md-12">
                      <p className="text-muted m-0">©ShopMyNeighborhood {new Date().getFullYear()}</p>
                    </div>
                  </div>
                </div>
              </div>
